...
for feat, (mn, mx, df_d) in {
    ...
    user_input[feat] = st.slider(feat, float(mn), float(mx), float(df_d))
...
if pred == 1:
    ...
    # Sound alert: play local alert.mp3 file continuously
    try:
        ...
        st.warning('Alert sound file (alert.mp3) not found.')

    # Flashing banner
    st.markdown(
        """
        <div style="background-color:#FFCCCC;padding:10px;border:2px solid red;animation: flash 1s infinite;">
          <strong>!!! WATER UNSAFE !!!</strong>
        </div>
        <style>
